...
async def test_list_memories_empty(client: AsyncClient):
    token = await _register_and_login(client, "mem_user_1")
...
async def test_create_memory(client: AsyncClient):
    token = await _register_and_login(client, "mem_user_2")
...
async def test_create_memory_shows_in_list(client: AsyncClient):
    token = await _register_and_login(client, "mem_user_3")
    await client.post(
        "/api/v1/memory/",
...
async def test_delete_memory(client: AsyncClient):
    token = await _register_and_login(client, "mem_user_4")
...
async def test_cannot_access_other_users_memories(client: AsyncClient):
    token1 = await _register_and_login(client, "mem_user_5a")
    token2 = await _register_and_login(client, "mem_user_5b")
...
async def test_memory_isolation_between_users(client: AsyncClient):
    ...
    token_a = await _register_and_login(client, "mem_user_6a")
    token_b = await _register_and_login(client, "mem_user_6b")
